...
bool Kosong(node *head, node *tail, int type){
    if (type == 1){
        if (head == NULL){
        return true;
        }
...
void Tambah_Data(node **head, node **tail, int type){
    int posisi;
    if (type == 3){
    ...
    for (int i = 0; i < nambah; i++){
        ...
        if (type == 1){ // ADD FIRST
            nodebaru->prev = NULL;
            nodebaru->next = NULL;
            if (Kosong(*head, *tail, 1)){// Jika Data Kosong
                *head = nodebaru;
                *tail = nodebaru;
            ...
            node *temp = *head;
            if (posisi == 1){
                if(Kosong(*head, *tail, 1)){//Jika data masih kosong
                    *head = nodebaru;
                    *tail = nodebaru;
                }else{
                    nodebaru->prev = NULL;
...
void Hapus_Data(node **head, node **tail, int type){
    if (Kosong(*head, *tail, 3)){
        cout << "[Linked List Masih Kosong . . . ]" << endl;
        system("pause");
        return;
    ...
    if (type == 1){ // Delete First
        ...
        *tail = (*tail)->prev;
        (*tail)->next = NULL;
        delete VarDelete;
    }else if (type == 3){// Delete Specifik
        node *temp = *head;
        int posisi;
        cout << "Hapus Data Pada Posisi : "; cin >> posisi;
        if (posisi == 1){
            Hapus_Data(*&head, *&tail, posisi);
            return;
        }else{
...
void Update_Data(node **head, node **tail){
    if(Kosong(*head, *tail, 3)){
        cout << "[Linked List Masih Kosong . . . ]" << endl;
        system("pause");
        return;
...
int main(){
    ...
    while (pilih != 0){
        ...
        cout << "[3] Tambah Data Tentara Awal" << endl;
        cout << "[4] Tambah Data Tentara Akhir" << endl;
        cout << "[5] Tambah Data Tentara Spesifik" << endl;
        cout << "[6] Hapus Data Tentara Awal" << endl;
        cout << "[7] Hapus Data Tentara Akhir" << endl;
        cout << "[8] Hapus Data Tentara Spesifik" << endl;
        cout << "[9] Edit Data Tentara" << endl;
        ...
        case 2:
            Tampilkan(head, tail, 2);system("Pause");break;
        case 3:
            Tampilkan(head, tail, 1);
            Tambah_Data(&head, &tail, 1);break;
        case 4:
            Tampilkan(head, tail, 1);
            Tambah_Data(&head, &tail, 2);break;
        case 5:
            Tampilkan(head, tail, 1);
            Tambah_Data(&head, &tail, 3);break;
        case 6:
            Tampilkan(head, tail, 1);
            Hapus_Data(&head, &tail, 1);break;
        case 7:
            Tampilkan(head, tail, 1);
            Hapus_Data(&head, &tail, 2);break;
        case 8:
            Tampilkan(head, tail, 1);
            Hapus_Data(&head, &tail, 3);break;
        case 9:
            Tampilkan(head, tail, 1);
            Update_Data(&head, &tail); break;
        case 10:
            sort();
